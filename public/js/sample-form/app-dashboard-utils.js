$(function() {
	...
	var ajax_submit_url = base_url +'ajax-calls/process-data-AJAX';
...
function _validateFormSubmission() {
	...
	// check for mandatory field(s)
		// var mandatory_fields = {'filter_bank': 'Bank is required', 'filter_percent_pos': 'Percentage-POS is required', 'filter_term_loan_amt': 'Term-Loan Amount is required', 'filter_term_loan_end_dt': 'Loan-Maturity-Date is required', 'filter_sec_deal_cde': 'Sec-Deal-Code is required'};
		var mandatory_fields = {'filter_percent_pos': 'Percentage-POS is required', 'filter_term_loan_amt': 'Term-Loan Amount is required', 'filter_sec_deal_cde': 'Sec-Deal-Code is required'};
		...
		// check for Occupation & Occupation-Percent
			...
			if( count_selected_occu>0 ) {
				
				if( count_selected_occu==1 ) {
					mandatory_fields['filter_occu_percent'] = 'Occupation Percentage is required';
				} else {
					if('filter_occu_percent' in mandatory_fields)
						delete mandatory_fields['filter_occu_percent'];	

					_unMarkError('filter_occu_percent');
				}
...
// for "Occupation/Percent/Sub-Occupation" change(s)
	$('#filter_occu').on('change', function() {
		
		var count_selected_occu = $(this).val().length;
		
		if( count_selected_occu>0 ) {
			
			if( count_selected_occu==1 ) {
				if( !$('#filter_occu_percent').is(":visible") )
					_showOrHideContainingDIV('filter_occu_percent', 'show');
			} else {
				_showOrHideContainingDIV('filter_occu_percent');
			}
		} else {
			_showOrHideContainingDIV('filter_occu_percent');
		}
...
function _load_existing_filters_AJAX() {
	...
	var ajax_URL = base_url +'ajax-calls/load-existing-filters-AJAX';
...
// form-submit event(s) [Begin]
	...
	function _resetForm() {
		...
		var DD_obj = {'filter_occu': 'Select Occupation(s)', 'filter_states': 'Select State(s)', 'filter_product_types': 'Select Products'};
		
		$.each(DD_obj, function(dd_id, dd_lbl) {
			
			dd_id = '#'+ dd_id;
			
			//$(dd_id).multiselect('clearSelection');
...
/* function to load 
 * - "Sub-Occupation(s)" based on selected "Occupation" */
function _load_sub_occupations(param_selected_occupation=null) {
	
	var modelSelect = $('#filter_sub_occu');
	var ajax_URL = base_url +'ajax-calls/load-sub-occupations-AJAX.php';
	
	$.ajax({
		type: "POST",
		url: ajax_URL,
		data: {'occu': param_selected_occupation},
		beforeSend: function(){
		...
		success: function(data) {
			...
			json_data = JSON.parse(data);
			modelSelect.html(json_data.html_output);
		}
...
function _load_mapped_sec_deal_code_AJAX() {
	...
	var ajax_URL = base_url +'ajax-calls/load-mapped-sec-deal-code-AJAX';

...
<div class="admin-content-header">
	<h1 class="float-left-sm primary-color">{{ $heading?? 'Sample Form' }}</h1>
</div>
		...
		<div class="col-la-12">
			<div class="card ">
					...
					<div class="card-container clearfix">
						...
						?>
						<form class="col-sm-12" id="frmAppNew" name="frmAppNew" method="POST" action="" role="search"  autocomplete="off">
							<input type="hidden" id="h_search" name="h_search" value="">
							<meta name="csrf-token" content="{{ csrf_token() }}">

							<div class="row" id="div_filters_block">
								<select name="filter_deal_code_mapping"   class="input-panel chosen-select">
...
<link rel="stylesheet" href="{{ asset('externalCSS/jquery-ui.css') }}" type="text/css" />
<link rel="stylesheet" href="{{ asset('css/sample-form/app-dashboard-styles.css') }}" type="text/css" />
<script type="text/javascript" src="{{ asset('js/sample-form/app-dashboard-utils.js') }}?ver=1.6"></script>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
		...
		$('#searchForm').on('submit',function(){
			...
			var b_valid = true;
			if (pageName.indexOf('/allApplications') !== -1)
			{
				if(user_access_type == 'ZONE' || user_access_type == 'ALL')
				{
					if( ($('#forFromDateSearch').val() == '' || $('#forFromDateSearch').val() == null) && ($('#forToDateSearch').val() == '' || $('#forToDateSearch').val() == null) && $('#allStatusSearch').val() == '' && $('#arohan_center_name').val() == '' && $('#groupCode').val() == '' && $('#arohan_customer_code').val() == '' && $('#fis_customer_code').val() == '' && $('#customer_name').val() == '' && $('#los_application_id').val() == '' && $('#zoneSearch').val() == ''){
						markAsError($('#btn_submit'),'You have to search using at least one filter!',1);
						b_valid = false;
					}
				}
				else
				{
					if( ($('#forFromDateSearch').val() == '' || $('#forFromDateSearch').val() == null) && ($('#forToDateSearch').val() == '' || $('#forToDateSearch').val() == null) && $('#allStatusSearch').val() == '' && $('#arohan_center_name').val() == '' && $('#groupCode').val() == '' && $('#arohan_customer_code').val() == '' && $('#fis_customer_code').val() == '' && $('#customer_name').val() == '' && $('#los_application_id').val() == ''){
						markAsError($('#btn_submit'),'You have to search using at least one filter!',1);
						b_valid = false;
					}
				}
	...
	$('.viewTransactionDetail').on('click', function(){
	       ...
	       var transactionID = $(this).attr("data-ID");
			...
			$.ajax({
				url: base_url+"unapproved-application/ajax_view_details",
	            type: "POST",
	            data: {
					groupCode: groupCode,

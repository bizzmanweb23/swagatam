...
<div class="admin-content-header">
    ...
    <div class=" float-right-sm">
        <a href="{{url('priorityassignment/edit')}}" class="button"><i class="material-icons">add</i>Add </a>
    </div>
</div>
        ...
        <div class="col-la-12">
            <div class="card ">                        
                <div class="card-container">
					...
					<div class="row"> 
						<form class="" id="searchForm" name="frm_search_2" method="POST" action="{{action('PriorityAssignmentController@list')}}" role="search">
							{{ csrf_field() }}
							...
							<div class="col-la-3 col-me-3 col-sm-6">
								<div class="form-panel">
									...
									<button type="reset" class="button button-gray" onclick="window.location.href='{{url('priorityassignment')}}'"><i class="material-icons">not_interested</i>Reset</button>
								</div>
							</div>
						</form>
                    </div>
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table-border no-margin table-head-color manage-aircraft-table table-odd-even">
                            <thead>
                                <tr>
                                    <th class="align-center text-nowrap">
                                        <a href="javascript:void(0);" onClick="sortData('{{url($pagination_url.'/'.$pageNo.'/priority_id/'.$sortT)}}')"> Priority {!! ($sortColmn == 'priority_id')?$sortIcon:$sortIconAll !!}</a>
                                    </th>
                            ...
                            <tr>
                            <td class="align-center text-nowrap @if($row->is_active == 0) only-read-view @endif">{{$row->priority_id}}</td>
                            ...
                            <td class="align-center action-tooltip tooltip-width">
								<div class="dropdownList">
                                    ...
                                    <div class="dropdownList-content">
                                        ...
                                        @if($row->is_active == 1)
                                            <a href="{{ url('priorityassignment/edit',$row->id) }}" class="icon-button button-primary" ><i class="fas fa-edit"></i><span> Edit</span></a>

                                            <a href="{{ url('priorityassignment/destroy', $row->id ) }}" class="icon-button button-red deletepopup" ><i class="fas fa-trash-alt"></i><span> Delete</span></a>
                                        @else
                                            <a href="{{ url('priorityassignment/restore', $row->id ) }}" class="icon-button button-red restorepopup" ><i class="fas fa-undo"></i><span> Restore</span></a>
                                        @endif        

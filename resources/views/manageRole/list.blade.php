...
<div class="admin-content-header">
    ...
    <div class=" float-right-sm">
        <a href="{{url('samadhanRole/edit')}}" class="button"><i class="material-icons">add</i>Add </a>
    </div>
</div>
        ...
        <div class="col-la-12">
            <div class="card ">                        
                <div class="card-container">
					...
					<div class="row"> 
						<form class="" id="searchForm" name="frm_search_2" method="POST" action="{{action('SamadhanRoleController@list')}}" role="search">
							{{ csrf_field() }}
							...
							<div class="col-la-2 col-me-3 col-sm-6">
								<div class="form-panel">
									<label>Name</label>
									<input type="text" placeholder="Role Name" class="input-panel" value="{{ $postVal['name'] }}" name="name" id="name">
								</div>
							</div>
							...
							<div class="col-la-3 col-me-3 col-sm-6">
								<div class="form-panel">
									...
									<button type="reset" class="button button-gray" onclick="window.location.href='{{url('samadhanRole')}}'"><i class="material-icons">not_interested</i>Reset</button>
								</div>
							</div>
						</form>
                    </div>
                    ...
                    <div class="table-responsive">
                        ...
                        <table class="table-border no-margin table-head-color manage-aircraft-table table-odd-even">
                            <thead>
                                <tr>
                                    <th class="align-center text-nowrap">
                                        <a href="javascript:void(0);" onClick="sortData('{{url($pagination_url.'/'.$pageNo.'/role_name/'.$sortT)}}')"> Role Name {!! ($sortColmn == 'role_name')?$sortIcon:$sortIconAll !!}</a>
                                    </th>
                                	...
                                	<th class="align-center text-nowrap">
										<a href="javascript:void(0);" onClick="sortData('{{url($pagination_url.'/'.$pageNo.'/type/'.$sortT)}}')"> Type {!! ($sortColmn == 'type')?$sortIcon:$sortIconAll !!}</a>
									</th>
                            ...
                            <tr>
                            <td class="align-left text-nowrap @if($row->is_active == 0) only-read-view @endif">{{$row->role_name}}</td>
                            <td class="align-left text-nowrap">{{ $row->long_name . " (".$row->short_name.")" }}</td>
                            <td class="align-left text-nowrap">{{ $row->type }}</td>
                            ...
                            <td class="align-center action-tooltip tooltip-width">
								<div class="dropdownList">
                                    ...
                                    <div class="dropdownList-content">
                                        ...
                                        @if($row->is_active == 1)
                                            <a href="{{ url('samadhanRole/edit',$row->id) }}" class="icon-button button-primary" ><i class="fas fa-edit"></i><span> Edit</span></a>

                                            <a href="{{ url('samadhanRole/destroy', $row->id ) }}" class="icon-button button-red deletepopup" ><i class="fas fa-trash-alt"></i><span> Delete</span></a>
                                        @else
                                            <a href="{{ url('samadhanRole/restore', $row->id ) }}" class="icon-button button-red restorepopup" ><i class="fas fa-undo"></i><span> Restore</span></a>
                                        @endif 
